...
export async function GET(request, { params }) {
  try {
    ...
    const response = await fetch(
      `${BACKEND_BASE_URL}/products/${id}/reviews?${new URLSearchParams({
        page,
        limit
      }).toString()}`,
      {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || 'Failed to fetch product reviews' },
...
export async function POST(request, { params }) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || 'Failed to add product review' },
        { status: response.status }
      );
    }
